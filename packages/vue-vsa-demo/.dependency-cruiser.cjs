...
module.exports = {
  forbidden: [
    ...
    },
    {
      ...
      comment: 'Slices must remain isolated. Use Public APIs or the Event Bus for communication.',
      severity: 'error',
      from: { path: '^src/modules/([^/]+)' },
      to: {
        path: '^src/modules/([^/]+)',
        pathNot: [
          '^src/modules/$1',         // Allow importing within the same slice
          '^src/modules/[^/]+/index.ts', // Allow importing via Public API index
          '^src/modules/[^/]+/[^/]+.public.ts' // Allow importing via .public.ts
        ]
      }
    },
    {
      name: 'no-module-imports-in-shared',
      comment: 'Shared utilities must be "leaf" nodes and cannot depend on business slices.',
      severity: 'error',
    ...
    },
    {
      ...
      comment: 'Private internal files should not be accessed from outside the slice.',
      severity: 'error',
      from: {
        pathNot: '^src/modules/([^/]+)'
      },
      to: {
        ...
        pathNot: [
          ...
          '^src/modules/[^/]+/contract.ts' // If you use separate contract files
        ]
      }
    }
  ],
...
// generated: dependency-cruiser@17.3.7 on 2026-02-06T04:37:36.119Z
